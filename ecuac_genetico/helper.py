...
class helper:
    ...
    def updateGraphs(i, population, results, ecuaParms):
    ...
    def createGraphs(population, results, ecuaParms):
        minIndx = results.index(min(results))
        # Saves the best in a global variable
        #global best 
        #best = (1, population[minIndx], results[minIndx])
        # History Figure
        global fig, ax 
        ...
        ax[0].axis([0, ecuaParms.iterations + 1, 0, 1.0])
        ...
        ax[1].title.set_text("Ecuations")
        axisRange = ecuaParms.maxR/ecuaParms.divisor
        ax[1].axis([0, 10, 0, 100])
        helper.printEcuation(ax, population[minIndx], axisRange, ecuaParms.divisor)
